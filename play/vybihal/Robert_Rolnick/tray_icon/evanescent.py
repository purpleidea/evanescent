...
class evanescent:
	...
	def eva_loop(self):
		...
		if os.name in ['posix']:
			import lpytraymsg
			msg = lpytraymsg.lpytraymsg()
			msg.create_tray(config.ICONIMAGE, 'evanescent')
			#Sample call to the tray icon:
			#msg.msg('Title','Body')
		if os.name in ['nt']:
					...
					for x in d:
						...
						if last_message < x['id']:
							...
							if (time.time() - x['tsync']) < config.STALETIME:
								...
								elif os.name in ['posix']: msg.msg('evanescent', x['msg'])	# send a pynotify msg
