				...
				for i in data:
					...
					for j in i.keys():
						...
						#*** effective group id
							...
							#may need to be in try/catch as well since it may raise same errors as time
							temp = dt.dt(time_shift=self.time_shift)
							...
							if not(fnmatch.fnmatchcase(self.c(socket.gethostbyaddr(socket.gethostname())[2][0]), self.c(i[j]))):
								row = False
								break
							else: row = True
