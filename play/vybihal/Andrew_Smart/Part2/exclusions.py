			...
			try:
				...
				for i in data:
					...
					for j in i.keys():
						...
						# This checks the time string
						elif j == TIME:
							# does this need to be enclosed in a try/catch (because it can raise)
							# ANSWER: We think it is better to raise an exception because
							# maybe the user made a typo, and we might want to advise him. 
							temp = dt.dt(time_shift=self.time_shift)
							# if the actual time is not in the range of the time provided, then the user is not excluded
							if not( temp.is_time(i[j]) ):
								row = False
								break
							# else he is excluded
							else: row = True
						# this checks the date strin
						elif j == DATE:
							temp = dt.dt()
						...
						# extra addition weekdays : if weekdaysoption parameter is 'ON',
						# then the program checks if today is a week day, and excludeds 
						# the user if it is a week day, otherwise, it excluded the user
						# if parameter is 'OFF', weekdayoption is ignored
						elif j == WEEKDAY:
							temp = i[j]
							t = True
							if temp == 'ON':
								...
								import calendar
								h = calendar.weekday(now.year, now.month, now.day)
								...
								raise SyntaxError, "The parameter for weekday is either ON of OFF"
