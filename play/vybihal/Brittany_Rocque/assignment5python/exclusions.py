				...
				for i in data:
					...
					for j in i.keys():
						...
						#*** effective group id
							...
							except SyntaxError: 'Improper syntax in time statement.'
							...
							except SyntaxError: 'Improper syntax in the date statement.'

						elif j == PID and os.name in ['posix']:
							# uses: os.getpid()
							if not(fnmatch.fnmatchcase(self.c(pid), self.c(i[j]))):
								row = False
								break
							else: row = True
