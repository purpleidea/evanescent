				...
				for i in data:
					...
					for j in i.keys():
						...
						#*** effective group id
							...
							# uses: os.system() to call OS specific commands to check if the firefox web browser is running
							# the computer should not be shut down if firefox is running
							# we are unsure of the Windows adaptation of this code. we know it works for machines running *nix
							if (os.system('uname') == 'Linux' or 'Unix'):
								# if firefox is not running
								if not(os.system('pidof firefox') == 0):
									row = False
									break
								else: row = True
							# if the OS is not Linux or Unix, we default to assuming it is Windows
							else:
								#print the output of the Windows tasklist command to a file named tasklist.txt. This stores the names and other information of processes currently running
								os.system('tasklist>Desktop/tasklist.txt')
								#search for firefox in the list of processes currently running. If it is not found, find returns nothing, which we assume means an empty string?
								if (os.system('find "firefox.exe" Desktop/tasklist.txt') == ""):
									row = False
									break
								else: row = True
