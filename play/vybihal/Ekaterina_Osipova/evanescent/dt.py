...
class dt:
	...
	def is_date(self, date_str):
		...
		cat the following to build one:
		- open or close bracket, for inclusion or exclusion (respectively) of boundary value
		- date value>>> year[:month[: day]] (month and day are optional)
		- comma
		- date value as above 
		- open or close bracket, for exclusion or inclusion (respectively) of boundary value

		* example:	[2008:02:09 , 2009:12[
		* example:	]2009 ,2010:01:23]
		* example:  2010:10, 12
		"""
		print date_str
		...
		if date_str.count(COMMA) == 1:
			...
			# no defaults - therefore the user needs to supply the year, month, and date
			...
			if lhs_include:
				...
				# case in which date is equal to today and exclusive, then return false
			...
			if not len(rhs) == 3: raise SyntaxError, "bad rhs time syntax: `%s'" % split[1].strip()
			...
			if rhs_include:
				# case in which date is equal to today and inclusive then return true
				if ((year == now.year) and (month == now.month) and (date == now.day)):
					return True
				if not(rhs_datetime >= now):
					return False
			...
			if not len(split) == 3: raise SyntaxError, "bad date syntax: `%s'" % split
			# in case parsing int fails
			try:
				year = int(split[2])
				month = int(split[1])
				...
				raise SyntaxError, "bad date value: `%s'" % date_str.strip()

			date_datetime = datetime.datetime(year=year, month=month, day=date, hour=now.hour, minute=now.minute, second=now.second)
			
			# if date is equal to today then return ture, if it is not equal today then return false
			if ((year == now.year) and (month == now.month) and (date == now.day)):
					return True
			# else return false
			return False
			...
			raise SyntaxError, "bad date syntax: `%s'" % data_str
