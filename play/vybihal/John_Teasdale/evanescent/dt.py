...
class dt:
	...
	def is_date(self, date_str):
		''' Returns true if the time currently is in the date range specified
                   the expected input format is
                   DDMMYYYY,DDMMYYYY     to exclude a date range 
               '''

		now = datetime.date().isoformat().translate(None, '-')	# now
