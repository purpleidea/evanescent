...
class dt:
	...
	def is_time(self, time_str):
		...
		* example:	9:44,12
		...
		if time_str.count(COMMA) == 1:
			...
			lhs = time_str.split(COLON)

			# in case parsing int fails
			try:
				hour=int(lhs[0])
			except ValueError:
				raise SyntaxError, "bad time value: `%s'" % time_str[0]

			lhs_hourtime = datetime.datetime(year=now.year, month=now.month, day=now.day, hour=hour, minute=0, second=0)

			if not(lhs_hourtime < now):
				return False
			...
			if not (hour2<23):
				hour2=hour2-24		
				rhs_hourtime=datetime.datetime(year=now.year, month=now.month, day=(now.day+1), hour=hour2, minute=0, second=0)
			else:
				rhs_hourtime=datetime.datetime(year=now.year, month=now.month, day=now.day, hour=hour2, minute=0, second=0)
	...
	def is_date(self, date_str):
		...
		if date_str.count(COMMA) == 1:
			...
			# need 3 chunks of date values to parse; eg: (x:y:z)
			...
			lhs_datetime = datetime.datetime(year=year, month=month, day=day, hour=0, minute=0, second=0)
			...
			# need 3 chunks of date values to parse; eg: (x:y:z)
			if len(rhs) != 3: raise SyntaxError, "bad rhs date syntax: `%s'" % split[1].strip()
			...
			try:
				...
				raise SyntaxError, "bad lhs date value: `%s'" % split[1].strip()
