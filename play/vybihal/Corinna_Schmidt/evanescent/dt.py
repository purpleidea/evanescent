...
class dt:
	...
	def is_date(self, date_str):
		...
		cat the following to build one:
		- open or close bracket, for inclusion or exclusion (respectively) of boundary value
		- date value>>> day/[month[/year]] (month and year are optional)
		- comma
		- date value as above
		- open or close bracket, for exclusion or inclusion (respectively) of boundary value
	
		* example: [07-03-2009 , 09-12-2010[
		* example: ]03-27 , 04-01]
		* example: 1997-01-16,2000-14-24
		"""

		left = True #True if the date is after the first boundary
		right = True #True if the date is before the second boundary				
		...
		if date_str.count(COMMA) == 1:
			...
			# defaults - not not specified, assume current month/year
			month = now.month
			year = now.year
			...
			# defaults - if not specified, assume current month and/or year
			month = now.month
			year = now.year
			...
			if (right & left): #The date is within the time range
				return True
			else:
				return False
