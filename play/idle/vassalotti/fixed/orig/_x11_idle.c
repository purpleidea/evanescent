...
static PyObject *sleep_until_idle(PyObject *self, PyObject *args) {
	...
	if (!PyArg_ParseTuple(args, "l", &seconds_before_idle)) return NULL;
	...
	if (!XScreenSaverQueryExtension(dpy, &event_basep, &error_basep)) {
		PyErr_SetString(PyExc_OSError, "XScreenSaver extension is not available");
		XCloseDisplay(dpy);
		return NULL;
	}
	...
	for (;;) {
		...
		if (!XScreenSaverQueryInfo(dpy, DefaultRootWindow(dpy), &saver_info)) {
			PyErr_SetString(PyExc_OSError, "unable to query XScreenSaver info");
			XCloseDisplay(dpy);
			return NULL;
		}
		...
		if (saver_info.idle / 1000 >= seconds_before_idle) break;
